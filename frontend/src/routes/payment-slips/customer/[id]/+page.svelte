<script lang="ts">
  ...
  export let data;
  let { customer }: { customer: { paymentSlips: any; naziv: any; } | null } = data;
  ...
  function fetchCustomer() {
    getCustomer($page?.params).then(() => getIsps());
  }
  ...
  onMount(async() => {
    fetchGlobalSetting();
    PaymentSlipTable = (await import("../../components/payment-slips.svelte")).default;
  })
</script>
