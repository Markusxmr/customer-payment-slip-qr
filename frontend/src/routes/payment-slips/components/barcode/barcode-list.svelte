<script lang="ts">
	...
	function emphasiseDate(val: string): string {
		if (!val) return '';
	...
	function emphasiseRate(val: string): string {
		if (!val) return '';
		let rule = / ([0-9])\/([0-9])/gm;
		let match = val.match(rule);
		return match && match.length === 1 ? val.replace(rule, `<strong>${match[0]}</strong>`) : val;
...
<div class="barcode-container">
	...
	<img src="/img/logo.png" alt="" class="barcode-list-logo" />
	<div class="row row-print">
		...
		{/each}
		<p class="text-center mt-0 barcode-list-support">Podrška 099/213-4567 Thomas K.</p>
	</div>
</div>
