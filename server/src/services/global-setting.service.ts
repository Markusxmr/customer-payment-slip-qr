...
import { CreateGlobalSettingDto } from '../modules/api/dto/global-setting/create-global-setting.dto';
import { UpdateGlobalSettingDto } from '../modules/api/dto/global-setting/update-global-setting.dto';
...
export class GlobalSettingService {
  ...
  async findOne(id: number) {
    ...
    let item = await this.globalSettingRepository.findOne({ where: { id } });
    if (items.length === 0) {
