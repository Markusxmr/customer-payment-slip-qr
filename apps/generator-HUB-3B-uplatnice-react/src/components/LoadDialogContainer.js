...
import { loadNalog } from "../redux/actions";
...
class LoadDialogContainer extends React.Component {
  ...
  loadFile(selectedFile) {
    ...
    reader.onload = (progressEv) => {
      ...
      try {
        ...
        this.showMsg(
          "Greška kod učitavanja datoteke. Format datoteke nije prepoznat...",
          "error"
        );
        return;
      }
  ...
  ucitajNalog() {
    if (this.props.onNalogLoad) {
      this.props.onNalogLoad(
        JSON.parse(localStorage.getItem(this.state.selectedKey))
      );
    }
...
const ConnectedLoadDialog = connect(
  null,
  mapDispatchToProps
)(LoadDialogContainer);
